...
router.post('/register', async (req, res) => {
  try {
    ...
    // Clean phone number format for WhatsApp (whatsapp:+123...)
    let cleanPhone = phone;
    if (cleanPhone && !cleanPhone.startsWith('whatsapp:')) {
      cleanPhone = `whatsapp:${cleanPhone.startsWith('+') ? '' : '+'}${cleanPhone}`;
    }
  ...
  try {
    ...
    if (!user && email.toLowerCase() === 'demo@student.com' && password === 'demo1234') {
      ...
      return res.json({
        success: true,
        token,
        user: { _id: 'demo-user-id', name: 'Demo Student', email: 'demo@student.com', college: 'Demo University', phone: 'whatsapp:+14155238886' }
      });
    }
...
router.get('/me', async (req, res) => {
  try {
    ...
    if (decoded.id === 'demo-user-id') {
      return res.json({ success: true, user: { _id: 'demo-user-id', name: 'Demo Student', email: 'demo@student.com', college: 'Demo University', phone: 'whatsapp:+14155238886', budgets: {} } });
    }
